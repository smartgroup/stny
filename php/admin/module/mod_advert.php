...
XML;
				...
				if( $type == '3' ) {
					//$input = array('gold', 'blue', 'green', 'ocean', 'aqua', 'ice', 'spark', 'silver', 'neon');
					//$k = array_rand($input);
			    	...
			    	//$fcolor2 = str_replace('#', '0x', $site_param['ADVERT_RTEXT_COLOR']);
...
<Content TheBannerWidth="'.$width.'" TheBannerHeight="'.$height.'" maxCellsize="1" NumberOfCells="30" CellsSpeed="1" CellsDirection="MoveDown" BackgroundImage="'.$bgimg2.'">';
			    	...
			    	/*$adtext2 = '<?xml version="1.0" encoding="UTF-8"?><messages><config stage_width="auto" stage_height="auto" /><image img_src="'.$bgimg2.'" />';
			    	
			    	// text list
					$txtArr2 = explode(';', $text2);
					$ln2 = count($txtArr2);
					for( $j=0; $j<$ln2; $j++ ) {
						$adtext2 .= '<text message_txt="'.$txtArr2[$j].'" text_url="'.$url2.'" text_size="'.$fsize2.'" text_color="'.$fcolor2.'" text_fade_speed="1" glow_color="'.$input[$k].'" glow_blur="4" glow_intensity="2" glow_quality="8" glow_width="100" glow_move_speed="5" glow_repeat_counter="2" />';
					}
					$adtext2 .= '</messages>';*/
