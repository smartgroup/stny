...
function JSDragDropTree() {
	...
	this.delNode = 'Delete';
	this.editNode = 'Edit';
	this.addNode = 'Add Child';
	this.imageFolder = '../script/mtree/theme/images/';
	...
	var filePathAddItem;
	var filePathEditItem;
	var modRewrite = '1'; // Dynamic mode
	...
	this.filePathAddItem = '';
	this.filePathEditItem = '';
...
JSDragDropTree.prototype = {
	...
	setCustomParam : function(param1, param2, param3) {
		if (param1.length) this.delNode = param1;
		if (param2.length) this.editNode = param2;
		if(param3!==undefined){
			if (param3.length) this.addNode = param3;
		}
	},
	...
	setFileNameAdd : function(newFileName, rewriteVal) {
		this.modRewrite = rewriteVal;
		this.filePathAddItem = newFileName;
	},
	setFileNameEdit : function(newFileName, rewriteVal) {
		this.modRewrite = rewriteVal;
	...
	createDropIndicator : function() {
		...
		img.src = this.imageFolder + 'drag_ind1.gif';
	...
	highlightItem : function(inputObj,e) {
		...
		if(ifm != null && subWeb != null) {
			ifm.style.height = parseInt(subWeb.body.scrollHeight)+70+'px';
			ifm.parentNode.style.height = parseInt(subWeb.body.scrollHeight)+80+'px';
		}
	},
	...
	addnodeItem : function(obj1,obj2) {
		var saveString = obj2.parentNode.id.replace(/[^0-9]/gi,'');
		var tag = (this.modRewrite == '1') ? '&' : '-';
		if(tag=="-"){
			JSTreeObj.filePathAddItem = JSTreeObj.filePathAddItem.replace(".html","");
			var tag2 = "-";			
		}else{
			tag2 = "=";
		}
		window.location.href = JSTreeObj.filePathAddItem + tag + "mi_id" + tag2 + saveString + ".html";
	},
	editnodeItem : function(obj1,obj2) {
		var saveString = obj2.parentNode.id.replace(/[^0-9]/gi,'');
		var tag = (this.modRewrite == '1') ? '&' : '-';
		if(tag=="-"){
			JSTreeObj.filePathEditItem = JSTreeObj.filePathEditItem.replace(".html","");
			var tag2 = "-";			
		}else{
			tag2 = "=";
		}
		window.location.href = JSTreeObj.filePathEditItem + tag + "mi_id" + tag2 + saveString + ".html";
	},
	...
	__deleteItem_step2 : function(obj) {
		...
		JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathDeleteItem;
		JSTreeObj.ajaxObjects[ajaxIndex].onCompletion = function() { JSTreeObj.__deleteComplete(ajaxIndex,obj); };
	...
	initTree : function() {
		...
		if(this.deleteAllowed){
			try{
				...
				if(this.deleteAllowed)menuModel.addItem(1,this.delNode,'','',false,'JSTreeObj.deleteItem');
				//if(this.renameAllowed)menuModel.addItem(2,'Rename','','',false,'JSTreeObj.renameItem');
				menuModel.addItem(2,this.editNode,'','',false,'JSTreeObj.editnodeItem');
				if(this.addNode!='Add Child'){
				menuModel.addItem(3,this.addNode,'','',false,'JSTreeObj.addnodeItem');
				}
				menuModel.init();	
				
				/*var menuModelRenameOnly = new DHTMLGoodies_menuModel();
				if(this.renameAllowed)menuModelRenameOnly.addItem(4,'Rename','','',false,'JSTreeObj.renameItem');
				menuModelRenameOnly.init();	*/
				var menuModelEditOnly = new DHTMLGoodies_menuModel();
				menuModelEditOnly.addItem(4,this.editNode,'','',false,'JSTreeObj.editnodeItem');
				menuModelEditOnly.init();

				var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
				if(this.deleteAllowed)menuModelDeleteOnly.addItem(5,this.delNode,'','',false,'JSTreeObj.deleteItem');
				menuModelDeleteOnly.init();	
				
				var menuModelAddOnly = new DHTMLGoodies_menuModel();
				menuModelAddOnly.addItem(6,this.addNode,'','',false,'JSTreeObj.addnodeItem');
