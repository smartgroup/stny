 ...
 **使用场景：GET方式请求时，对URL的中文进行编码
	...
	*return 拼接完成以后的字符串
*/
function create_linkstring_utf8($array) {
    $arg  = "";
    while (list ($key, $val) = each ($array)) {
		if ($key == "subject" || $key == "body" || $key == "receive_name" || $key == "receive_address" || $key == "receive_zip" || $key == "receive_phone" || $key == "receive_mobile")
